...
@tool
def generate_pdf_report_tool(analysis_data: str) -> str:
    """
    將分析結果整理成正式的 PDF 報告。
    輸入應包含：原始序列、最佳變體、理化數據、檔案路徑等資訊。
    """
    try:
        pdf = FPDF()
        pdf.add_page()
        
        # 設定字體 (如果需要顯示中文，請確保系統有對應的 .ttf 檔案)
        pdf.set_font("Arial", 'B', 16)
        pdf.cell(200, 10, txt="Protein Analysis Research Report", ln=True, align='C')
        ...
        # 寫入分析內容
        pdf.ln(5)
        pdf.multi_cell(0, 10, txt=analysis_data)
        
        # 存檔
        report_name = f"Report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
        pdf.output(report_name)
        
        return f"✅ PDF 報告已生成：{report_name}"
    except Exception as e:
...
agent = initialize_agent(
    ...
    agent_kwargs={
        ...
        你的作業流程如下：
        1. 接收序列後，優先使用 full_auto_analysis_tool 進行全面分析。
        2. 分析完成後，必須將得到的數據（序列、MW、pI、檔案路徑）彙整。
        3. 最後，主動呼叫 generate_pdf_report_tool 將這些整理好的資訊輸出成 PDF 報告給使用者。
        請確保報告內容詳盡且易於閱讀。"""
    }
)
