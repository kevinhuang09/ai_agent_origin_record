...
def do_prediction(sequence: str) -> dict:
    ...
    try:
        ...
        with open(csv_path, mode='r', encoding='utf-8') as f:
            ...
            raw_score = (latest_row.get('Average_Score')
                         or latest_row.get('probability')
                         or latest_row.get('score') or "0.0")
            try:
...
# 🟢 修正：同時提供 /ankh 與 /ankh_feature 兩個入口（向後相容）
@app.post("/ankh")
