...
app = FastAPI(title="Peptide Prediction Server")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
# 🧠 通用預測腳本執行器
# ==========================================
def run_predictor_script(
    sequence: str,
    script_name: str,
    csv_name: str,
    note: str,
    fasta_name: str = "test.fasta",
    timeout: int = 300
) -> dict:
    """
    通用的預測腳本呼叫器。
    ...
    Args:
        sequence: 蛋白質序列
        script_name: 預測腳本檔名 (e.g. "predict_web.py")
        csv_name: 預測結果 CSV 檔名
        note: 結果中要附加的備註
        fasta_name: 寫入的 FASTA 檔名（要對應到腳本內讀取的檔名）
        timeout: 子程序最大執行時間（秒）
    """
    fasta_path = os.path.join(MAIN_PROGRAM_DIR, fasta_name)
    csv_path = os.path.join(MAIN_PROGRAM_DIR, csv_name)
    script_abs_path = os.path.join(MAIN_PROGRAM_DIR, script_name)

    # 🔍 Debug 訊息
    print(f"\n{'='*50}")
    print(f"[Predictor] 序列: {sequence[:30]}{'...' if len(sequence) > 30 else ''}")
    print(f"[Predictor] FASTA: {fasta_path}")
    print(f"[Predictor] 腳本: {script_abs_path}")
    print(f"[Predictor] 腳本存在: {os.path.exists(script_abs_path)}")
    print(f"{'='*50}")
    ...
    try:
        ...
        # 寫入 fasta（header 用 seq1，與 CSV 對應）
        ...
        print(f"[Predictor] returncode: {result.returncode}")
        if result.stdout:
            print(f"[Predictor] stdout: {result.stdout[-300:]}")
        if result.stderr:
            print(f"[Predictor] stderr: {result.stderr[-300:]}")
...
async def predict(data: dict):
    ...
    res = run_predictor_script(
        sequence=sequence,
        script_name="predict_web.py",
        csv_name="final_prediction_results.csv",
        note="AI Predicted",
...
async def anti_aging_predictor(data: dict):
    ...
    res = run_predictor_script(
        sequence=sequence,
        script_name="anti_aging_predictor.py",
        csv_name="anti_aging_prediction_result.csv",
        note="AI Predicted (Anti-Aging)",
        fasta_name="anti_aging_test.fasta"
    )
    if res["label"] == "error":
        return {"status": "error", "message": res["note"]}
    return {"status": "success", "prediction": res}
...
@app.post("/anti_hemo_predictor")
async def anti_hemo_predictor(data: dict):
    """抗溶血預測"""
    sequence = data.get("sequence", "").strip().upper()
    if not sequence:
        return {"status": "error", "message": "No sequence"}
    ...
    res = run_predictor_script(
        sequence=sequence,
        script_name="anti_hemo_main_predict.py",
        csv_name="anti_hemo_prediction_result.csv",
        note="AI Predicted (Anti-Hemolytic)",
        fasta_name="anti_hemo_test.fasta"
    )
    if res["label"] == "error":
        return {"status": "error", "message": res["note"]}
    return {"status": "success", "prediction": res}
...
async def secondary_structure(data: dict):
    ...
    try:
        ...
        return {"status": "success", "sequence": sequence,
                "structure": structure, "summary": summary}
    except Exception as e:
        return {"status": "error", "message": str(e)}
