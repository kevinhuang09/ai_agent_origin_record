...
class ChatGPTStyleGUI:
    # ChatGPT 風格配色
    BG_MAIN = "#343541"          # 主背景（深灰）
    BG_SIDEBAR = "#202123"       # 側邊欄
    BG_USER_MSG = "#343541"      # 使用者訊息背景
    BG_BOT_MSG = "#444654"       # AI 訊息背景
    BG_INPUT = "#40414F"         # 輸入框
    FG_TEXT = "#ECECF1"          # 主文字
    FG_SECONDARY = "#8E8EA0"     # 次要文字
    ACCENT = "#10A37F"           # ChatGPT 綠
    BORDER = "#565869"
    ...
    def __init__(self, root):
        ...
        # === 字型（conda Tk 限制下使用點陣字） ===
        def _pick_font():
            return "fangsong ti"

        _family = _pick_font()
        # ⭐ 字體加大，提升可讀性
        self.font_main  = tkfont.Font(family=_family, size=13)
        self.font_bold  = tkfont.Font(family=_family, size=13, weight="bold")
        self.font_title = tkfont.Font(family=_family, size=16, weight="bold")
        self.font_small = tkfont.Font(family=_family, size=11)
    ...
    def _build_ui(self):
        ...
        # === 左側欄 ===
        sidebar = tk.Frame(main_container, bg=self.BG_SIDEBAR, width=280)
        sidebar.pack(side=tk.LEFT, fill=tk.Y)
        sidebar.pack_propagate(False)

        # 標題（◆ 取代 🧬）
        title_frame = tk.Frame(sidebar, bg=self.BG_SIDEBAR)
        title_frame.pack(fill=tk.X, padx=18, pady=20)
        tk.Label(title_frame, text="◆  Protein Agent",
                 font=self.font_title, bg=self.BG_SIDEBAR,
                 fg=self.FG_TEXT).pack(anchor=tk.W)
        tk.Label(title_frame, text="AI 蛋白質研究助理",
                 font=self.font_small, bg=self.BG_SIDEBAR,
                 fg=self.FG_SECONDARY).pack(anchor=tk.W, pady=(6, 0))  # ⭐ 副標題上下間距

        # 新對話按鈕（＋ 取代）
        new_chat_btn = tk.Button(sidebar, text="＋   新對話",
                                  font=self.font_main,
                                  bg=self.BG_SIDEBAR, fg=self.FG_TEXT,
                                  activebackground="#2A2B32",
        ...
        new_chat_btn.pack(fill=tk.X, padx=18, pady=6, ipady=10)  # ⭐ 按鈕間距加大

        # 生成 PDF 按鈕（▤ 取代 📄）
        pdf_btn = tk.Button(sidebar, text="▤   生成 PDF 報告",
                             font=self.font_main,
                             bg=self.BG_SIDEBAR, fg=self.FG_TEXT,
                             activebackground="#2A2B32",
                             activeforeground=self.FG_TEXT,
        ...
        pdf_btn.pack(fill=tk.X, padx=18, pady=6, ipady=10)

        # 分隔線
        tk.Frame(sidebar, bg=self.BORDER, height=1).pack(fill=tk.X, padx=18, pady=15)

        # 範例指令（◇ 取代 💡）
        tk.Label(sidebar, text="◇   範例指令", font=self.font_bold,
                 bg=self.BG_SIDEBAR, fg=self.FG_SECONDARY).pack(
                 anchor=tk.W, padx=18, pady=(8, 8))
        ...
        for ex in examples:
            btn = tk.Button(sidebar, text="  ▸  " + ex, font=self.font_small,
            ...
            btn.pack(fill=tk.X, padx=18, pady=4, ipady=6)  # ⭐ 範例間距

        # 底部資訊
        bottom_frame = tk.Frame(sidebar, bg=self.BG_SIDEBAR)
        bottom_frame.pack(side=tk.BOTTOM, fill=tk.X, padx=18, pady=18)
        tk.Frame(bottom_frame, bg=self.BORDER, height=1).pack(fill=tk.X, pady=(0, 12))
        tk.Label(bottom_frame, text="◉   黃瑞麟",
                 font=self.font_main, bg=self.BG_SIDEBAR,
                 fg=self.FG_TEXT).pack(anchor=tk.W)
        tk.Label(bottom_frame, text="Powered by Gemma2:9b",
                 font=self.font_small, bg=self.BG_SIDEBAR,
                 fg=self.FG_SECONDARY).pack(anchor=tk.W, pady=(6, 0))

        # === 右側聊天區 ===
        chat_area = tk.Frame(main_container, bg=self.BG_MAIN)
        chat_area.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)

        # 訊息顯示區（Canvas + Scrollbar）
        chat_wrapper = tk.Frame(chat_area, bg=self.BG_MAIN)
        chat_wrapper.pack(fill=tk.BOTH, expand=True)

        self.canvas = tk.Canvas(chat_wrapper, bg=self.BG_MAIN, highlightthickness=0)
        scrollbar = ttk.Scrollbar(chat_wrapper, orient=tk.VERTICAL, command=self.canvas.yview)
        self.canvas.configure(yscrollcommand=scrollbar.set)

        scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        self.canvas.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)

        self.msg_frame = tk.Frame(self.canvas, bg=self.BG_MAIN)
        ...
        # === 底部輸入區 ===
        input_outer = tk.Frame(chat_area, bg=self.BG_MAIN)
        input_outer.pack(fill=tk.X, padx=24, pady=(0, 24))  # ⭐ 加大邊距

        input_container = tk.Frame(input_outer, bg=self.BG_INPUT,
                                    highlightbackground=self.BORDER,
                                    highlightthickness=1)
        input_container.pack(fill=tk.X)

        # 輸入框（多行，加大高度）
        self.input_text = tk.Text(input_container, height=3,  # ⭐ 2 → 3
                                   font=self.font_main,
                                   bg=self.BG_INPUT, fg=self.FG_TEXT,
                                   insertbackground=self.FG_TEXT,
                                   relief=tk.FLAT, bd=0,
                                   wrap=tk.WORD, padx=18, pady=14,
                                   spacing1=4, spacing2=4, spacing3=4)  # ⭐ 加行距
        ...
        # 狀態列
        self.status_label = tk.Label(input_outer,
                                      text="按 Enter 傳送   |   Shift + Enter 換行",
                                      font=self.font_small,
                                      bg=self.BG_MAIN, fg=self.FG_SECONDARY)
        self.status_label.pack(pady=(12, 0))  # ⭐ 跟輸入框拉開
    ...
    def append_user_message(self, text):
        row = tk.Frame(self.msg_frame, bg=self.BG_USER_MSG)
        row.pack(fill=tk.X)

        # ⭐ pady 加大：訊息與訊息之間更寬鬆
        inner = tk.Frame(row, bg=self.BG_USER_MSG)
        inner.pack(fill=tk.X, padx=60, pady=24)

        # 頭像（◉ 取代 👤）
        avatar = tk.Label(inner, text="◉", font=("fixed", 18, "bold"),
                          bg="#5436DA", fg="white", width=3, height=1)
        avatar.pack(side=tk.LEFT, anchor="n", padx=(0, 18))

        # 內容
        content_frame = tk.Frame(inner, bg=self.BG_USER_MSG)
        content_frame.pack(side=tk.LEFT, fill=tk.X, expand=True)

        tk.Label(content_frame, text="您",
                 font=self.font_bold, bg=self.BG_USER_MSG,
                 fg=self.FG_TEXT).pack(anchor=tk.W)

        # ⭐ 改用 Text 加 spacing 讓行距變大
        line_count = text.count('\n') + max(1, len(text) // 70)
        height = min(max(line_count, 1), 20)

        msg_text = tk.Text(content_frame, font=self.font_main,
                           bg=self.BG_USER_MSG, fg=self.FG_TEXT,
                           relief=tk.FLAT, bd=0, wrap=tk.WORD,
                           height=height, padx=0, pady=0,
                           spacing1=4, spacing2=6, spacing3=4,  # ⭐ 行距
                           highlightthickness=0)
        msg_text.insert("1.0", text)
        msg_text.configure(state=tk.DISABLED)
        msg_text.pack(anchor=tk.W, pady=(8, 0), fill=tk.X)

        self._scroll_to_bottom()

    def append_bot_message(self, text, is_log=False):
        row = tk.Frame(self.msg_frame, bg=self.BG_BOT_MSG)
        row.pack(fill=tk.X)

        # ⭐ pady 加大
        inner = tk.Frame(row, bg=self.BG_BOT_MSG)
        inner.pack(fill=tk.X, padx=60, pady=24)

        # 頭像（◆ / ⚙ 取代）
        bg_avatar = "#10A37F" if not is_log else "#8E8EA0"
        symbol = "◆" if not is_log else "⚙"
        avatar = tk.Label(inner, text=symbol, font=("fixed", 18, "bold"),
                          bg=bg_avatar, fg="white", width=3, height=1)
        avatar.pack(side=tk.LEFT, anchor="n", padx=(0, 18))

        # 內容
        content_frame = tk.Frame(inner, bg=self.BG_BOT_MSG)
        content_frame.pack(side=tk.LEFT, fill=tk.X, expand=True)

        name = "Protein Agent" if not is_log else "系統日誌"
        tk.Label(content_frame, text=name,
                 font=self.font_bold, bg=self.BG_BOT_MSG,
                 fg=self.FG_TEXT).pack(anchor=tk.W)

        # ⭐ 行高估算更寬鬆，行距明顯加大
        line_count = text.count('\n') + max(1, len(text) // 70)
        height = min(max(line_count + 2, 3), 35)  # ⭐ +2 給呼吸空間

        msg_text = tk.Text(content_frame, font=self.font_main,
                           bg=self.BG_BOT_MSG, fg=self.FG_TEXT,
                           relief=tk.FLAT, bd=0, wrap=tk.WORD,
                           height=height, padx=0, pady=0,
                           spacing1=4, spacing2=8, spacing3=4,  # ⭐ 行距 8 解決字疊在一起
                           highlightthickness=0)
        msg_text.insert("1.0", text)
        msg_text.configure(state=tk.DISABLED)
        msg_text.pack(anchor=tk.W, pady=(8, 0), fill=tk.X)
        ...
        for img_path in png_matches:
            if os.path.exists(img_path):
                try:
                    ...
                    img_label.pack(anchor=tk.W, pady=(14, 0))
                except Exception as e:
                    print(f"圖片載入失敗: {e}")
        ...
        try:
            while True:
                ...
                if msg_type == "bot":
                    ...
                    self.send_btn.configure(state=tk.NORMAL, text="→")
